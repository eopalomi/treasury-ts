...
import { paymentExceptions, PaymentDetailExceptions } from '../../domain/exceptions/payment.exceptions';

export class PaymentController {
  constructor(
    private createPaymentUseCase: CreatePaymentUseCase,
    private findPaymentUseCase: FindPaymentUseCase,
    private updatePaymnetUseCase: UpdatePaymentUseCase
  ) { }

  createPayment = async ({ body }: Request, res: Response) => {
    try {
      const payment =
        await this.createPaymentUseCase.createNonTraditionalPayment(
          {
      ...
      if (
        error instanceof paymentExceptions ||
        error instanceof PaymentDetailExceptions
      ) {
        const errorCodes = {
          currencyType: '01',
  ...
  findbyIdPayment = async ({ params }: Request, res: Response) => {
    try {
      const idPayment: number = parseInt(params.id);

      const payment = await this.findPaymentUseCase.findPayment(idPayment);
  ...
  updatePayment = async ({ body, params }: Request, res: Response) => {
    ...
    try {
      const searchPayment = await this.findPaymentUseCase.findPayment(
        parseInt(params.id)
      );
